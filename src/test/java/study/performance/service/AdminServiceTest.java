...
class AdminServiceTest {
    ...
    void test4() {
        ...
        //when
        PerformanceInformationResponseDto response = adminService.get(performance.getId());

        //then
        assertThat(response).isNotNull();
        assertThat(performanceRepository.count()).isEqualTo(1);
        assertThat(response.getPerformanceName()).isEqualTo(name);
        assertThat(response.getPerformanceNumber()).isEqualTo(number);
        assertThat(response.getPerformanceLocation()).isEqualTo(location);
        assertThat(response.getStartDate()).isEqualTo(startDate);
        assertThat(response.getEndDate()).isEqualTo(endDate);
        assertThat(response.getRuntime()).isEqualTo(runtime);
        assertThat(response.getAge()).isEqualTo(age);
        assertThat(response.getPrice()).isEqualTo(price);
        assertThat(response.getThumbnail()).isEqualTo(thumbnail);
    }
}